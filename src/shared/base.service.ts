...
export class BaseService<T extends Typegoose> {
    ...
    async deleteContacts(filter = {}, item_id): Promise<InstanceType<T>> {
        const subdocObj = {contacts: item_id};
        return this._model.findOneAndUpdate(filter, {$pull: subdocObj}, {new: true}).exec();
    }

    async updateConversation(id: string, item_id): Promise<InstanceType<T>> {
        //conversations: [ {participants: User[] }]
        return this._model.findByIdAndUpdate(this.toObjectId(id), {$addToSet: {conversations: [id, item_id]}}, {new: true}).exec()
    }
